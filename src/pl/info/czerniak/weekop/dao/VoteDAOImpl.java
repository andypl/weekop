...
public class VoteDAOImpl implements VoteDAO {
    ...
    private static final String READ_VOTE = "SELECT vote_id, discovery_id, user_id, type FROM vote WHERE vote_id = :vote_id;";
    private static final String READ_VOTE_BY_DISCOVERY_USE_IDS = "SELECT vote_id, discovery_id, user_id, date, type FROM vote WHERE user_id = :user_id AND discovery_id = :discovery_id;";
    ...
    public Vote create(Vote vote) {
        ...
        int update = template.update(CREATE_VOTE, parameterSource, keyHolder, new String[] {"vote_id"});
        if(update > 0){
            System.out.println("Vote key: " + keyHolder.getKey().longValue());
            voteCopy.setId(keyHolder.getKey().longValue());
        }
        return voteCopy;
    }
    ...
    public boolean update(Vote vote) {
        ...
        paramMap.put("date", vote.getDate());
        paramMap.put("type", vote.getVoteType().toString());
        paramMap.put("vote_id", vote.getUserId());
        SqlParameterSource parameterSource = new MapSqlParameterSource(paramMap);
