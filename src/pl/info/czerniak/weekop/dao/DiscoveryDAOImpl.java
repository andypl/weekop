...
public class DiscoveryDAOImpl implements DiscoveryDAO {
    ...
    private static final String READ_DISCOVERY = "SELECT users.user_id, username, email, is_active, password, discovery_id, name, description, url, date, up_vote, down_vote "
            + "FROM discovery LEFT JOIN users ON discovery.user_id=users.user_id WHERE discovery_id=:discovery_id;";
    private static final String READ_ALL_DISCOVERIES =
            "SELECT users.user_id, username, email, is_active, password, discovery_id, name, description, url, date, up_vote, down_vote "
                    + "FROM discovery LEFT JOIN users ON discovery.user_id=users.user_id;";
    ...
    public Discovery create(Discovery discovery) {
        ...
        int update = template.update(CREATE_DISCOVERY, parameterSource,keyHolder, new String[] {"discovery_id"});
        if(update > 0){
    ...
    public boolean update(Discovery discovery) {
        ...
        paramMap.put("down_vote", discovery.getDownVote());
        SqlParameterSource parameterSource = new MapSqlParameterSource(paramMap);
        int update = template.update(UPDATE_DISCOVERY, parameterSource);
        if(update > 0){
            result = true;
        }
        return result;
